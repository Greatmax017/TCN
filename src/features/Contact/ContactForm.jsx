...
import { Input, Textarea, Button, FormControl, FormLabel, Select, FormErrorMessage, VStack, SimpleGrid, Icon } from "@chakra-ui/react";
...
function ContactForm() {
  ...
  const inputs = [
...

    {
      ...
      options: ["Service Inquiries", "Giving", "Others"],
    },
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit(handleSendContactForm)} style={{ width: "100%" }} method="post">
        <SimpleGrid w="full" columns={[1, 1, 2]} gap="10px">
          {inputs.map((input, i) => (
            <FormControl
              key={i}
              id={input.name}
            ...
            >
              ...
              <FormErrorMessage>{errors[input.name] && errors[input.name].message}</FormErrorMessage>
            </FormControl>
          ))}
        </SimpleGrid>

        <VStack w="full" justify="end" align="end">
          <Button
            type="submit"
            // w="full"
            bg="brand.100"
